...
class TextNode:
    ...
    @classmethod
    def split_nodes_delimiter(cls, old_nodes, delimiter, text_type):
        new_nodes = []
        for node in old_nodes:
            if node.text_type != TextType.TEXT or delimiter not in node.text:
                ...
                for i, part in enumerate(node.text.split(delimiter)):
                    if part:
                        new_nodes.append(cls(part, TextType.TEXT if i % 2 == 0 else text_type))
        return new_nodes

    @classmethod
    def split_nodes_pattern(cls, old_nodes, pattern, text_type, text_format):
        new_nodes = []
        for node in old_nodes:
            if node.text_type != TextType.TEXT:
                ...
                while len(node_text) > 0:
                    ...
                    if items:
                        ...
                        if pos > 0:
                            new_nodes.append(cls(node_text[:pos], TextType.TEXT))
                        new_nodes.append(cls(item[0], text_type, item[1]))
                        node_text = node_text[pos + len(item_text) :]
                    else:
                        new_nodes.append(cls(node_text, TextType.TEXT))
                        break
        return new_nodes
